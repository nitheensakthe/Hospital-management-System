...
const STATUS_ACTIONS = {
  pending:   [{ label: 'Confirm',  next: 'confirmed', color: '#10b981' }, { label: 'Cancel', next: 'cancelled', color: '#ef4444' }],
  confirmed: [{ label: 'Complete', next: 'completed', color: '#4f46e5' }, { label: 'Cancel', next: 'cancelled', color: '#ef4444' }],
  completed: [],
  cancelled: [],
};
...
function AppointmentCard({ apt, expanded, onToggle, view, userRole, onStatusChange }) {
  const dateObj  = new Date(apt.date);
  const dateStr  = dateObj.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric', year: 'numeric' });
  const monthStr = dateObj.toLocaleDateString('en-US', { month: 'short' });
  const dayStr   = dateObj.getDate();
  const actions  = (userRole === 'doctor' || userRole === 'admin') ? (STATUS_ACTIONS[apt.status] || []) : [];
        ...
        <div className="ap-card-body">
          <div className="ap-detail-chips">
            <div className="ap-detail-chip">
              ...
              <div><div className="ap-chip-label">Date</div><div className="ap-chip-value">{dateStr}</div></div>
            </div>
            <div className="ap-detail-chip">
              <span className="ap-chip-icon">👨⚕️</span>
              <div><div className="ap-chip-label">Doctor</div><div className="ap-chip-value">{apt.doctor || 'To be assigned'}</div></div>
            </div>
            <div className="ap-detail-chip">
              <span className="ap-chip-icon">👤</span>
              <div><div className="ap-chip-label">Patient</div><div className="ap-chip-value">{apt.patientName}</div></div>
            </div>
            <div className="ap-detail-chip">
              <span className="ap-chip-icon">🏷️</span>
              <div><div className="ap-chip-label">Status</div><div className="ap-chip-value" style={{ textTransform: 'capitalize' }}>{apt.status}</div></div>
            </div>
          </div>

          <div className="ap-reason-section">
            <div className="ap-section-header"><span>📝</span><span className="ap-section-title">Reason for Visit</span></div>
            <p className="ap-reason-text">{apt.reason}</p>
          </div>

          {/* Status action buttons for doctor/admin */}
          {actions.length > 0 && (
            <div className="ap-status-actions">
              <span className="ap-status-actions-label">Update Status:</span>
              {actions.map(a => (
                <button key={a.next} className="ap-status-btn"
                  style={{ '--btn-color': a.color }}
                  onClick={e => { e.stopPropagation(); onStatusChange(apt.id, a.next); }}>
                  {a.label}
                </button>
              ))}
            </div>
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.post('/appointments', {
        date: formData.date, reason: formData.reason,
        doctorId: formData.doctorId || null, doctor: formData.doctor || null
      });
    ...
    <div className="dashboard">
      ...
      <div className="dashboard-content">
        <div className="ap-page-header">
          ...
          {user?.role === 'patient' && (
            <button onClick={() => setShowModal(true)} className="btn-primary ap-book-btn">+ Book Appointment</button>
          )}
        </div>
        ...
        <div className="ap-toolbar">
          <div className="ap-search-wrap">
            ...
            <input className="ap-search-input" type="text"
          ...
          <div className="ap-view-toggle">
            <button className={`ap-view-btn ${view === 'grid' ? 'active' : ''}`} onClick={() => setView('grid')}>⊞</button>
            <button className={`ap-view-btn ${view === 'list' ? 'active' : ''}`} onClick={() => setView('list')}>☰</button>
          </div>
        </div>

        <div className="ap-tabs">
          {STATUS_TABS.map(tab => (
            <button key={tab.key} className={`ap-tab ${activeTab === tab.key ? 'active' : ''}`}
        ...
        {filtered.length === 0 ? (
          <div className="ap-empty">
            ...
            {user?.role === 'patient' && !search && activeTab === 'all' && (
              <button onClick={() => setShowModal(true)} className="btn-primary" style={{ marginTop: 20 }}>+ Book First Appointment</button>
            )}
          </div>
        ) : (
          <div className={`ap-list ${view}`}>
            {filtered.map(apt => (
              <AppointmentCard key={apt.id} apt={apt} view={view}
        ...
        {filtered.length > 0 && (
          <p className="ap-results-count">Showing {filtered.length} of {appointments.length} appointment{appointments.length !== 1 ? 's' : ''}</p>
        )}
      </div>
        ...
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal-content ap-modal" onClick={e => e.stopPropagation()}>
            <div className="ap-modal-header">
              ...
              <div><h2>Book Appointment</h2><p>Fill in the details to schedule your visit</p></div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="ap-modal-form">
              <div className="form-group">
                ...
                <input ref={firstFieldRef} type="date" value={formData.date}
                  min={new Date().toISOString().split('T')[0]} required
                  onChange={e => { setError(''); setFormData({ ...formData, date: e.target.value }); }} />
              </div>
              <div className="form-group">
                <label>Reason for Visit <span className="ap-char-count">({formData.reason.length}/{REASON_LIMIT})</span></label>
                <textarea value={formData.reason} rows={3} required
                  placeholder="e.g., Regular checkup, Consultation, Follow-up…"
                  maxLength={REASON_LIMIT}
                  onChange={e => { setError(''); if (e.target.value.length <= REASON_LIMIT) setFormData({ ...formData, reason: e.target.value }); }} />
              </div>
              <div className="ap-doctor-row">
                <div className="form-group">
                  <label>Choose Doctor <span className="ap-optional">(optional)</span></label>
                  <select value={formData.doctorId} disabled={loadingDoctors}
                ...
                <div className="form-group">
                  ...
                  <input type="text" value={formData.doctor} placeholder="Custom doctor name"
